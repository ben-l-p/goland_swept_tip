...
class FlyingWing():
    ...
    def update_fem_prop(self):
        ...
        ### Nodal coordinates
        n_main = int(pos_frac_h[0][0]*(num_node_surf+1))    #Number of nodes on main wing (including kink)
        n_tip = num_node_surf - n_main + 1                  #Number of nodes on wing tip (including kink)
        pos_h = half_span*pos_frac_h[0][0]

        z = np.zeros((num_node_tot,))

        if n_surfaces == 1:
            ### Local coord from half surface
            # x01_main = np.sin(sweep_rad) * np.linspace(0., pos_h, node_kink)
            # x01_tip = np.sin(sweep_rad+ang_h[0][0]) * np.linspace(0, half_span - pos_h, num_node_surf-node_kink+1) + x01_main[-1]
            # x01 = np.concatenate([x01_main, x01_tip[1:]])
            
            # y01_main = np.cos(sweep_rad) * np.linspace(0., pos_h, node_kink)
            # y01_tip = np.cos(sweep_rad+ang_h[0][0]) * np.linspace(0, half_span - pos_h, num_node_surf-node_kink+1) +y01_main[-1]
            # y01 = np.concatenate([y01_main, y01_tip[1:]])

            x1_main = np.linspace(0, 0, n_main)
            x1_tip = np.linspace(0, half_span-pos_h, n_tip)[1:]*np.sin(ang_h[0][0])
            x1 = np.concatenate([x1_main, x1_tip])

            y1_main = np.linspace(0, pos_h, n_main)
